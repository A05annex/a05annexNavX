 ...
 * heading, for the robot. We have been having a degrees vs. radians debate for a while. Since all of the
 * math trig libraries use radians. we decided that staying in radians would build up minimal round-off error.
 * <p>
 * Originally this class was written to support NavX on a conventional drive that had PID direction loops
 * for matching actual heading to expected heading (2018, 2019 and 2020 seasons). For 2021 we switched to swerve
 * drive and use the NavX to determine the required chassis heading based on the driver's requested
 * field heading.
 */
public class NavX {
    ...
    public enum Type {
        NAVX_MXP {
            public String toString() { return "navX-MXP"; }
        },
        NAVX_MICRO {
            public String toString() { return "navX-Micro"; }
        },
        NAVX2_MXP {
            public String toString() { return "navX2-MXP"; }
        },
        NAVX2_MICRO {
            public String toString() { return "navX2-Micro"; }
        },
        TEST {
    ...
    private static class TypeInfo {
        final long m_maxInitializationMs;
        final boolean m_useTestInstance;

        TypeInfo(long maxInitializationMs, boolean useTestInstance) {
            m_maxInitializationMs = maxInitializationMs;
            m_useTestInstance = useTestInstance;
        }
    }

    static EnumMap<Type,TypeInfo> typeMap = new EnumMap<>(Type.class);

    static {
        typeMap.put(Type.NAVX_MXP, new TypeInfo(25000, false));
        typeMap.put(Type.NAVX_MICRO, new TypeInfo(25000, false));
        typeMap.put(Type.NAVX2_MXP, new TypeInfo(8000, false));
        typeMap.put(Type.NAVX2_MICRO, new TypeInfo(8000, false));
        typeMap.put(Type.TEST, new TypeInfo(25000, true));
        typeMap.put(Type.TEST2, new TypeInfo(8000, true));
    }
    ...
    public static class NavXConfig {
        final private String m_type;
        final private SPI.Port m_spi;
        final private I2C.Port m_ic2;
        final private SerialPort.Port m_serial;
        final private int m_orientation;
        final private AHRS m_testAHRS;
        public NavXConfig(String type, @Nullable SPI.Port spi_port_id, @Nullable I2C.Port i2c_port_id,
                          @Nullable SerialPort.Port serial_port_id, int orientation, @Nullable AHRS testAHRS) {
            m_type = type;
            m_spi = spi_port_id;
            m_ic2 = i2c_port_id;
            m_serial = serial_port_id;
            m_orientation = orientation;
            m_testAHRS = testAHRS;
        }
    }

    static protected class WrapAHRS {
        AHRS  m_ahrs;
        /**
         * The heading we are trying to track with the robot (in degrees)
         */
        double m_expectedHeading = 0.0;
        double m_updateCt = -1;

        /**
         * The raw heading read fom the NavX, not corrected for the spins, read directly from the NavX, in the range
         * -180 to +180. Used for determining whether the boundary between -180 and 180 has been crossed.
         */
        double m_headingRawLast = 0.0;

        /**
         * The number of complete revolutions the robot has made since the NavX wa initialized..
         */
        int m_headingRevs = 0;
        ...
        boolean m_setExpectedToCurrent = false;
        double m_refPitch = 0.0;
        double m_refYaw = 0.0;
        double m_refRoll = 0.0;
        double m_refHeading = 0.0;

    }

    static class InitAHRS implements Runnable {
        final NavX m_parent;
        final NavXConfig m_config;
        InitAHRS(final NavX parent, final NavXConfig config) {
            m_parent = parent;
            m_config = config;
        }
        ...
        @Override
        public void run() {
            WrapAHRS ahrsWrapper = new WrapAHRS()
            ahrsWrapper.m_ahrs = new AHRS(SPI.Port.kMXP);
            ahrsWrapper.m_ahrs.reset();
            while (ahrsWrapper.m_ahrs.isCalibrating()) {
                try {
                    Thread.sleep(100);
            ...
            ahrsWrapper.m_updateCt = ahrsWrapper.m_ahrs.getUpdateCount();
     ...
     *
     */
    static private boolean s_configSpecified = false;
    static private List<NavXConfig> s_configs = null;
    private List<WrapAHRS> m_ahrs;
    /**
     *
     */
    /**
    ...
    private NavX(List<NavXConfig> configs) {
        ...
        for (NavXConfig config : configs) {
            // what we do here is start an initialization thread for each leisted AHRS
    ...
    /**
     * Sets the reference start heading and navigation reference positions to the current values. NOTE: this
     * assumes the robot is heading directly downfield (heading = 0&deg;;away from the drivers) when the
     * robot is initialized. This should be called immediately at the start of autonomous.
     */
    ...
    public static synchronized void configure(List<NavXConfig> configs) {
        s_configs = configs;
        s_configSpecified = true;
        s_instance = null;
    }
     ...
     *
     * @return Return the NavX instance.
     */
    @Nullable
    public static synchronized NavX getInstance() {
        if ((null == s_instance) && s_configSpecified) {
            s_instance = new NavX(s_configs);
        }
        return s_instance;
    }
}
